...
- Распознавать пользовательские сообщения из `message.parts[*].text`.
- Распознавать ответы ИИ с ролью `model` как assistant-сообщения.
- Исключать telemetry, tool calls и tool responses из основного диалога.
- Автоматически прокручивать выбранный чат к последним сообщениям.
- Позволять продолжать выбранный диалог через `--resume <session_id>`.
- Показывать ход выполнения GigaCode во время продолжения диалога и сворачивать его после завершения.
- Скролл списка чатов и скролл выбранного диалога должны быть независимыми от скролла страницы.
